...
public class AutoScrollViewPager extends ViewPager {
	...
	public void startAutoScroll() {
		startAutoScroll(intervalInMillis != 0 ? intervalInMillis
				: DEFAULT_INTERNAL_IM_MILLIS);
	}
	...
	public void setAdapter(PagerAdapter adapter) {
		...
		wrapperPagerAdapter = (wrappedPagerAdapter == null) ? null
				: new AutoScrollPagerAdapter(adapter);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		Log.i("AutoScrollViewPager", "onTouchEvent");
	...
	private void setScrollerIfNeeded() {
		...
		try {
			...
			scroller = new AutoScrollFactorScroller(getContext(),
	...
	private class InnerOnPageChangeListener implements OnPageChangeListener {
		...
		public void onPageScrolled(int position, float positionOffset,
			...
			if (listener != null && position > 0 && position < getCount()) {
				listener.onPageScrolled(position - 1, positionOffset,
		...
		public void onPageSelected(final int position) {
			// if (listener != null && position != 0 && position <
			// wrappedPagerAdapter.getCount() + 1) {
			if (listener != null) {
				final int pos;
				// Fix position
				if (position == 0) {
				...
				// Comment this, onPageSelected will be triggered twice for
				// position 0 and getCount -1.
				// Uncomment this, PageIndicator will have trouble.
				// if (lastSelectedPage != pos) {
				lastSelectedPage = pos;
				// Post a Runnable in order to be compatible with
				// ViewPagerIndicator because
